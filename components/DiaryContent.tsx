...
import { getMonthGrid, toDateStr, isToday, isSameDay } from '@/lib/calendar'
  ...
  return (
    <main className="px-8 py-10" style={{ maxWidth: '960px', paddingBottom: '80px' }}>
      ...
      <div className="mb-8">
        ...
        <p style={{ fontSize: '15px', color: '#7a7a7a', marginTop: '4px' }}>날짜를 클릭해서 일기를 작성하세요</p>
      </div>
      ...
      <div className={`gap-6 ${selectedDate ? 'grid' : ''}`} style={selectedDate ? { gridTemplateColumns: '1fr 420px' } : {}}>
        ...
        >
          ...
          <div className="grid grid-cols-7 mb-2">
            {DAY_LABELS.map((d) => (
              <div key={d} className="text-center py-1" style={{ fontSize: '12px', fontWeight: 600, color: '#7a7a7a' }}>
                {d}
              </div>
            ))}
          </div>

          <div className="rounded-[12px] overflow-hidden" style={{ border: '1px solid #f0f0f0' }}>
            {grid.map((week, wi) => (
              <div key={wi} className="grid grid-cols-7" style={{ borderBottom: wi < 5 ? '1px solid #f0f0f0' : 'none' }}>
                {week.map((day) => {
                  ...
                  return (
                    <button
                      ...
                      className="flex flex-col items-center justify-start pt-2 pb-2 min-h-[72px] transition-colors hover:bg-[#f5f5f7]"
                      style={{
                        backgroundColor: selected ? 'rgba(0,102,204,0.06)' : '#ffffff',
                    ...
                    >
                      <span
                        ...
                        style={{
                          ...
                          color: today ? '#ffffff' : inMonth ? '#1d1d1f' : '#d2d2d7',
          ...
          <div className="flex gap-3 mt-4 flex-wrap">
            {MOODS.map(({ emoji, label }) => (
              <span key={label} className="flex items-center gap-1" style={{ fontSize: '12px', color: '#7a7a7a' }}>
                {emoji} {label}
              </span>
            ))}
          </div>
        ...
        {selectedDate && (
          <div
            ...
            style={{ backgroundColor: '#ffffff', border: '1px solid #e0e0e0', minHeight: '500px' }}
          >
            <div className="flex items-start justify-between mb-4">
              <div>
                <p style={{ fontSize: '13px', color: '#7a7a7a', marginBottom: '2px' }}>{selectedDateLabel}</p>
                {entry?.updatedAt && (
                  <p style={{ fontSize: '11px', color: '#b0b0b5' }}>
                    마지막 저장: {new Date(entry.updatedAt).toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' })}
                  </p>
                )}
              </div>
              <button
                onClick={() => { setSelectedDate(null); router.replace('/diary', { scroll: false }) }}
                style={{ fontSize: '20px', color: '#7a7a7a', lineHeight: 1 }}
              >
                ×
              </button>
            </div>
            ...
            <div className="flex gap-2 mb-4">
              {MOODS.map(({ value, emoji, label }) => (
                <button
                  ...
                  style={{
                    backgroundColor: editMood === value ? 'rgba(0,102,204,0.1)' : '#f5f5f7',
                    border: editMood === value ? '1px solid rgba(0,102,204,0.3)' : '1px solid transparent',
                  }}
                >
                  <span style={{ fontSize: '20px' }}>{emoji}</span>
                  <span style={{ fontSize: '10px', color: editMood === value ? '#0066cc' : '#7a7a7a' }}>{label}</span>
                </button>
              ))}
            </div>
            ...
            <div className="flex items-center justify-between mt-4 pt-4" style={{ borderTop: '1px solid #f0f0f0' }}>
              ...
              <button
                ...
                style={{
                  fontSize: '14px',
                  fontWeight: 600,
                  backgroundColor: dirty ? '#0066cc' : '#f0f0f0',
