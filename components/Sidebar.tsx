        ...
        <nav className="mt-1 flex flex-col gap-0.5 px-2">
          {DIARY_ITEMS.map((item) => {
            if ('children' in item) {
              ...
              return (
                <div key={item.label}>
                  <div className="flex items-center gap-2 px-3 py-2" style={{ fontSize: '14px', fontWeight: groupActive ? 600 : 400, color: groupActive ? '#0066cc' : 'var(--text-primary)' }}>
                  ...
                  {item.children.map(({ label, href }) => {
                    ...
                    return (
                      <Link key={href} href={href} className="flex items-center gap-2 py-1.5 rounded-[8px] transition-colors" style={{ fontSize: '13px', fontWeight: active ? 600 : 400, color: active ? '#0066cc' : 'var(--text-secondary)', backgroundColor: active ? 'rgba(0,102,204,0.08)' : 'transparent', paddingLeft: '28px', paddingRight: '12px' }}>
            ...
            return (
              <Link key={item.href} href={item.href} className="flex items-center gap-2 px-3 py-2 rounded-[8px] transition-colors" style={{ fontSize: '14px', fontWeight: active ? 600 : 400, color: active ? '#0066cc' : 'var(--text-primary)', backgroundColor: active ? 'rgba(0,102,204,0.1)' : 'transparent' }}>
                <span className="w-1.5 h-1.5 rounded-full flex-shrink-0" style={{ backgroundColor: active ? '#0066cc' : '#d2d2d7' }} />
                {item.label}
              </Link>
            )
